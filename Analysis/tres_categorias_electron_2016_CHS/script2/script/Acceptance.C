...
//    Float_t bins_gen[] = {-2.,-1.,0.,1.,2.}; 
    ...
    treereco_ttbar->Project("Var_gen",Form("%s < %f ? TMath::Max(%f,%s): (%s > %f ? TMath::Min(%f,%s) : %s)",var_gen.c_str(),bins_gen[0]+0.01,bins_gen[0]+0.01,var_gen.c_str(),var_gen.c_str(),bins_gen[binnum_gen]-0.01,bins_gen[binnum_gen]-0.01,var_gen.c_str(),var_gen.c_str()),"( (ttagN == 0 && btagN >= 1 && wtagN == 1) ||  (ttagN == 1 && btagN >= 1 && wtagN == 0) || (ttagN == 0 && btagN >= 1 && wtagN == 0)) &&  rec_chi2 < 30 && Mttbar > 900"); 
