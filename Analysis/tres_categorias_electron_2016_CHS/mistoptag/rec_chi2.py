...
_fileDir ="/nfs/dust/cms/user/hugobg/ZPrime_102X/analysis_output/2016_CHS/electron/"
...
for sample in sample_names:
		...
		legendR.AddEntry(hist1_[sample],sample,'f')       
        	hist1_[sample].SetYTitle(histograms[histName][1])        
                ...
                tree_MC_up[sample].Draw("%s>>h3_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"weight*weight_sfelec_TightID_up*weight_sfelec_Trigger_up*weight_pu_up*weight_toptagSF_up_*1*weight_sfelec_Rec_up*(%s && btagN==0 && rec_chi2 > 30)"%(argv[2]))
