...
for sample in sample_names:
		...
		tree_MC[sample]=_file[sample].Get("AnalysisTree")
 	        ...
 	        tree_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*weight_pu*weight_toptagSF_*weight_pt_rew*weight_btagdisc_central*muonrecSF_nominal*1.15*(ttagN >= 0 && wtagN >= 0 && btagN>=0 && rec_chi2 < 300 && Mttbar < 10000)")
		...
		legendR.AddEntry(hist1_[sample],sample,'f')       
        	hist1_[sample].SetYTitle(histograms[histName][1])        
                ...
                tree_MC_up[sample].Draw("%s>>h3_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"weight*weight_sfmu_HighPtID_up*weight_sfmu_Trigger_up*weight_pu_up*weight_toptagSF_up_*1.25*muonrecSF_up*(ttagN >= 0 && wtagN >= 0 && btagN>=0 && rec_chi2 < 300 && Mttbar < 10000)")
