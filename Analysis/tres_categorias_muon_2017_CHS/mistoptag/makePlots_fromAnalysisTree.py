...
stackList = {"TTToHadronic_2017":[kRed-3], "TTToOthers":[kRed+2], "TTToSemiLeptonic_2017":[kRed],"DYJetsToLL_M-50_HT_2017":[kBlue], "QCD_HT_2017":[kTeal], "WJetsToLNu_2017":[kGreen], "ST_2017":[kYellow], "WW_WZ_ZZ_2017":[kOrange]}
...
sample_names = ["DYJetsToLL_M-50_HT_2017", "QCD_HT_2017", "WJetsToLNu_2017", "ST_2017", "WW_WZ_ZZ_2017", "TTToSemiLeptonic_2017","TTToOthers"]
...
for sample in sample_names:
		...
		tree_MC[sample]=_file[sample].Get("AnalysisTree")
 	        ...
 	        tree_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*weight_pu*weight_toptagSF_*weight_pt_rew*weight_btagdisc_central*muonrecSF_nominal*(ttagN == 0 && wtagN == 0 && btagN==0 && rec_chi2 > 30 && Mttbar < 4000)")
		...
		legendR.AddEntry(hist1_[sample],sample,'f')       
        	hist1_[sample].SetYTitle(histograms[histName][1])        
                ...
                tree_MC_up[sample].Draw("%s>>h3_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"weight*weight_sfmu_HighPtID_up*weight_sfmu_Trigger_up*weight_pu_up*weight_toptagSF_up_*1.05*muonrecSF_up*(ttagN == 0 && wtagN == 0 && btagN==0 && rec_chi2 > 30 && Mttbar < 4000)")
...
tree.Draw("%s>>dat_hist(%i,%i,%f)"%(histName,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(ttagN == 0 && wtagN == 0 && btagN==0 && rec_chi2 > 30 && Mttbar < 4000)")
