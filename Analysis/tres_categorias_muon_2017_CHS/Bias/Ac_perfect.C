...

{
    ...
    chreco->Add("/nfs/dust/cms/user/hugobg/ZPrime_102X/analysis_output/2017_CHS/muon/uhh2.AnalysisModuleRunner.MC.TTToS*.root");
...
//rapidity
    treereco->Project("Smearing_Matrix_closure",Form("%s < -2. ? TMath::Max(-1.99,%s): (%s > 2. ? TMath::Min(1.99,%s) : %s):%s < -2. ? TMath::Max(-1.99,%s): (%s > 2. ? TMath::Min(1.99,%s) : %s)",rap_diff.c_str(),rap_diff.c_str(),rap_diff.c_str(),rap_diff.c_str(),rap_diff.c_str(),"DeltaY","DeltaY","DeltaY","DeltaY","DeltaY"),"","",250000,1);
    treereco->Project("DeltaY_gen_closure",Form("%s < -2. ? TMath::Max(-1.99,%s): (%s > 2. ? TMath::Min(1.99,%s) : %s)",rap_diff.c_str(),rap_diff.c_str(),rap_diff.c_str(),rap_diff.c_str(),rap_diff.c_str()),"","",250000,250001);
    treereco->Project("DeltaY_rec_closure",Form("%s < -2. ? TMath::Max(-1.99,%s): (%s > 2. ? TMath::Min(1.99,%s) : %s)","DeltaY","DeltaY","DeltaY","DeltaY","DeltaY"),"","",250000,250001);
