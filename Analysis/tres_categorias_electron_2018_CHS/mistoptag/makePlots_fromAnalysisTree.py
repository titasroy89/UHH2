...
for sample in sample_names:
		...
		tree2_MC[sample]=_file[sample].Get("AnalysisTree")
	        tree2_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"weight*weight_pu*weight_sfelec_TightID*weight_sfelec_Trigger*weight_pt_rew*weight_btagdisc_central*1.0*(btagN>=1 && rec_chi2 < 30 && Mttbar < 4000)")
...
tree.Draw("%s>>dat_hist(%i,%i,%f)"%(histName,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(wtagN >= 0 && btagN>=1 && rec_chi2 < 30 && Mttbar < 4000)")
