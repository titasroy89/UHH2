...
for sample in sample_names:
		...
		tree2_MC[sample]=_file[sample].Get("AnalysisTree")
	        tree2_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"weight*weight_pu*weight_sfmu_HighPtID*weight_sfmu_Trigger*weight_btagdisc_central*weight_pt_rew*0.77*(wtagN==0 && ttagN == 1 && btagN>=1 && rec_chi2 < 30)")
...
tree.Draw("%s>>dat_hist(%i,%i,%f)"%(histName,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(wtagN==0 && ttagN == 1 && btagN>=1 && rec_chi2 < 30)")
