...
_fileDir ="/nfs/dust/cms/user/hugobg/Zprime_102X/analysis_output/2016/muon/workdir_Zprime_Analysis_2016_muon_ttag_btag/"
...
stackList = { "TTbar":[kRed], "TTbar_up":[kGreen], "TTbar_down":[kYellow]}
...
#             "met_pt"   : ["missing E_{T} [GeV]", "Events", 10, [50,1500]],
              "Mttbar": ["Mttbar", "Events", 60, [0, 6000]], 
...
sample_names = ["TTbar", "TTbar_up", "TTbar_down"]

for sample in sample_names:
        	print sample, histName
		_file[sample] = TFile("%s/uhh2.AnalysisModuleRunner.MC.TTbar.root"%(_fileDir),"read")
        	print "%s/uhh2.AnalysisModuleRunner.MC.TTbar.root"%(_fileDir)
		tree2_MC[sample]=_file[sample].Get("AnalysisTree")
                if(sample == "TTbar"):
			tree2_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(ttagN == 0 & btagN == 1)*weight*weight_pu*weight_btagdisc_central*0.75")   
                if(sample == "TTbar_up"):
	                tree2_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(ttagN == 0 & btagN == 1)*weight*weight_pu_up*weight_btagdisc_central*0.75")
                if(sample == "TTbar_down"):
                        tree2_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(ttagN == 0 & btagN == 1)*weight*weight_pu_down*weight_btagdisc_central*0.75")
...
for sample in sample_names:
    hist1_[sample].Draw("SAME,P")

canvas.Print("DeltaY_new_sys.pdf")
...
if log:
	...
	canvasRatio.SaveAs("%s_new_sys.pdf"%(histName))
