...
*/
    ...
    chreco_ttbar_chs->Add("/nfs/dust/cms/user/hugobg/ZPrime_102X/analysis_output/2018_CHS/muon/uhh2.AnalysisModuleRunner.MC.TTToS*.root/AnalysisTree");
    TTree *treereco_ttbar_chs = (TTree*) chreco_ttbar_chs;

    TChain *chreco_ttbarh_chs = new TChain("AnalysisTree","");
    chreco_ttbarh_chs->Add("/nfs/dust/cms/user/hugobg/ZPrime_102X/analysis_output/2018_CHS/muon/uhh2.AnalysisModuleRunner.MC.TTToH*.root/AnalysisTree");
    ...
    chreco_wjets_chs->Add("/nfs/dust/cms/user/hugobg/ZPrime_102X/analysis_output/2018_CHS/muon/uhh2.AnalysisModuleRunner.MC.WJ*.root/AnalysisTree");
    TTree *treereco_wjets_chs = (TTree*) chreco_wjets_chs;

    TChain *chreco_ST_chs = new TChain("AnalysisTree","");
    chreco_ST_chs->Add("/nfs/dust/cms/user/hugobg/ZPrime_102X/analysis_output/2018_CHS/muon/uhh2.AnalysisModuleRunner.MC.ST*.root/AnalysisTree");
    TTree *treereco_ST_chs = (TTree*) chreco_ST_chs;

    TChain *chreco_DY_chs = new TChain("AnalysisTree","");
    chreco_DY_chs->Add("/nfs/dust/cms/user/hugobg/ZPrime_102X/analysis_output/2018_CHS/muon/uhh2.AnalysisModuleRunner.MC.DYJets*.root/AnalysisTree");
    TTree *treereco_DY_chs = (TTree*) chreco_DY_chs;

    TChain *chreco_QCD_chs = new TChain("AnalysisTree","");
    chreco_QCD_chs->Add("/nfs/dust/cms/user/hugobg/ZPrime_102X/analysis_output/2018_CHS/muon/uhh2.AnalysisModuleRunner.MC.QCD*.root/AnalysisTree");
    TTree *treereco_QCD_chs = (TTree*) chreco_QCD_chs;

    TChain *chreco_data_chs = new TChain("AnalysisTree","");
    chreco_data_chs->Add("/nfs/dust/cms/user/hugobg/ZPrime_102X/analysis_output/2018_CHS/muon/uhh2.AnalysisModuleRunner.DATA.DATA*.root/AnalysisTree");
...
//---------selection_cuts && weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*s --------??
    ...
    string selcuts_data_boosted = Form("(Mttbar> 900 && ttagN == 1 && btagN >= 1 && wtagN == 0 && %s)",region.c_str());  
    string selcuts_data_semiresolved = Form("(Mttbar > 900 && ttagN == 0 && btagN >= 1 && wtagN == 1 && %s)",region.c_str());
    string selcuts_data_resolved = Form("(Mttbar > 900 && ttagN == 0 && btagN >= 1 && wtagN == 0 && %s)",region.c_str());

    Float_t mistag_boosted_chs = 0.70;

    Float_t mistag_semiresolved_chs = 0.7;

    Float_t mistag_resolved_chs = 1.;

    string selcuts_boosted_chs_central = Form("(Mttbar > 900 && ttagN == 1 && btagN >= 1 && wtagN == 0 && %s)*%f",region.c_str(),mistag_boosted);

    string selcuts_semiresolved_chs_central = Form("(Mttbar > 900 && ttagN == 0 && btagN >= 1 && wtagN == 1 && %s)*%f",region.c_str(),mistag_semiresolved);

    string selcuts_resolved_chs_central = Form("(Mttbar > 900 && ttagN == 0 && btagN >= 1 && wtagN == 0 && %s)*%f",region.c_str(),mistag_resolved);

    string selcuts_data_boosted_chs = Form("(ttagN == 1 && btagN >= 1 && Mttbar > 900. && wtagN == 0 && %s)",region.c_str());
...
*/
    ...
    treereco_ttbar_chs->Project("TTbar_resolved_chs","Ak4_j1_eta",Form("%s*weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*weight_pu*weight_pt_rew*weight_btagdisc_central",selcuts_resolved_chs_central.c_str()));
    treereco_ttbarh_chs->Project("TTbar_resolvedh_chs","Ak4_j1_eta",Form("%s*weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*weight_pu*weight_pt_rew*weight_btagdisc_central",selcuts_resolved_chs_central.c_str()));
    treereco_ttbar2l_chs->Project("TTbar_resolved2l_chs","Ak4_j1_eta",Form("%s*weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*weight_pu*weight_pt_rew*weight_btagdisc_central",selcuts_resolved_chs_central.c_str()));
    treereco_wjets_chs->Project("WJets_resolved_chs","Ak4_j1_eta",Form("%s*weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*weight_pu*weight_pt_rew*weight_btagdisc_central",selcuts_resolved_chs_central.c_str()));
    treereco_ST_chs->Project("ST_resolved_chs","Ak4_j1_eta",Form("%s*weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*weight_pu*weight_pt_rew*weight_btagdisc_central",selcuts_resolved_chs_central.c_str()));
    treereco_DY_chs->Project("DY_resolved_chs","Ak4_j1_eta",Form("%s*weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*weight_pu*weight_pt_rew*weight_btagdisc_central",selcuts_resolved_chs_central.c_str()));
    treereco_QCD_chs->Project("QCD_resolved_chs","Ak4_j1_eta",Form("%s*weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*weight_pu*weight_pt_rew*weight_btagdisc_central",selcuts_resolved_chs_central.c_str()));
