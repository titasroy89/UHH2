...
//---------selection_cuts && weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*s --------??
    ...
    if (region == "ttagN == 1 && btagN >= 1 && wtagN == 0 && rec_chi2 < 30 && Mttbar > 900"){
        mistoptag = 0.82;
        mistoptag_up = 0.87;
        mistoptag_down = 0.77;
    }
    ...
    string selcuts_central = Form("(%s)*%f",region.c_str(),mistoptag);
    string selcuts_up = Form("(%s)*%f",region.c_str(),mistoptag_up);
    string selcuts_down = Form("(%s)*%f",region.c_str(),mistoptag_down);

    string selcuts_data = Form("(%s)",region.c_str());  
