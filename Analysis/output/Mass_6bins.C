...
//    TTree *treereco1 = (TTree*) chreco1;
    ...
    chreco2->Add("/nfs/dust/cms/user/hugobg/ZPrimeSemi/Preselection_2016/workdir_Preselection/uhh2.AnalysisModuleRunner.MC.TT_TuneCUETP8M2T4_2016v3.root");
    TTree *treereco2 = (TTree*) chreco2;


//    Float_t bins_gen[] = {840,1000,1350,1550,1822,2200,2600,7000};
//    Float_t bins_rec[] = {840,1000,1350,1550,1822,2200,2600,7000};
//    Float_t new_rec[] = {840,920,1000,1100,1350,1450,1550,1686,1822,2011,2200,2400,2600,4300,7000};

//    Float_t bins_gen[] = {850,925,1100,1350,1550,1822,2200,2600,7000};
//    Float_t bins_rec[] = {850,925,1100,1350,1550,1822,2200,2600,7000};
//    Float_t new_rec[] = {850,887,925,1025,1100,1225,1350,1450,1550,1686,1822,2011,2200,2400,2600,4300,7000};

//    Float_t bins_gen[] = {850,1100,1350,1550,1822,2200,2600,6000};
//    Float_t bins_rec[] = {850,1100,1350,1550,1822,2200,2600,6000};
//    Float_t new_rec[] = {850,975,1100,1225,1350,1450,1550,1686,1822,2011,2200,2400,2600,3800,6000};

    Float_t bins_gen[] = {850,1070,1300,1500,1800,2200,2600,6000};
    Float_t bins_rec[] = {850,1070,1300,1500,1800,2200,2600,6000};
    Float_t new_rec[] = {850,960,1070,1185,1300,1400,1500,1650,1800,2000,2200,2400,2600,3300,6000};

//    Float_t bins_gen[] = {850,1070,1300,1500,1800,2200,2600,6000};
//    Float_t bins_rec[] = {850,1070,1300,1500,1800,2200,2600,6000};
//    Float_t new_rec[] = {850,960,1070,1185,1300,1400,1500,1650,1800,2000,2200,2600,3000,4500,6000};
...
//Llenando histogramas para closure test
    ...
    treereco->Project("Smearing_Matrix_closure","Mttbar:TMath::Sqrt(TMath::Power(GenParticles.m_energy[2] + GenParticles.m_energy[3],2) - TMath::Power(GenParticles.m_pt[2]*TMath::Cos(GenParticles.m_phi[2]) + GenParticles.m_pt[3]*TMath::Cos(GenParticles.m_phi[3]),2) - TMath::Power(GenParticles.m_pt[2]*TMath::Sin(GenParticles.m_phi[2]) + GenParticles.m_pt[3]*TMath::Sin(GenParticles.m_phi[3]),2) - TMath::Power(GenParticles.m_pt[2]*TMath::SinH(GenParticles.m_eta[2]) + GenParticles.m_pt[3]*TMath::SinH(GenParticles.m_eta[3]),2))","","",14000,14001);  
    treereco->Project("Mttbar_gen_closure","TMath::Sqrt(TMath::Power(GenParticles.m_energy[2] + GenParticles.m_energy[3],2) - TMath::Power(GenParticles.m_pt[2]*TMath::Cos(GenParticles.m_phi[2]) + GenParticles.m_pt[3]*TMath::Cos(GenParticles.m_phi[3]),2) - TMath::Power(GenParticles.m_pt[2]*TMath::Sin(GenParticles.m_phi[2]) + GenParticles.m_pt[3]*TMath::Sin(GenParticles.m_phi[3]),2) - TMath::Power(GenParticles.m_pt[2]*TMath::SinH(GenParticles.m_eta[2]) + GenParticles.m_pt[3]*TMath::SinH(GenParticles.m_eta[3]),2))","","",14000,1);
    treereco->Project("Mttbar_rec_closure","Mttbar","","",14000,1);
...
//    Acceptance->Draw("same");
    ...
    Double_t Res_gauss[] = {306,206,166,247,307,617,1100};
...
/*
    TCanvas* Gaus = new TCanvas("Gaus","Gaus",2400,1200);
    Gaus->Divide(1,1);

    Double_t Res_gauss[] = {306,206,166,247,307,617,1100};
    Double_t RMS[] = {320,242,259,323,433,653,1222};
    Double_t Bin_size[binnum_gen];
    Double_t Relative_Resolution[binnum_gen];
    Double_t Relative_RMS[binnum_gen];

    TH1F *Ratio_res = new TH1F("Ratio_res","",binnum_gen,bins_gen);
    TH1F *Ratio_RMS = new TH1F("Ratio_RMS","",binnum_gen,bins_gen);

    for(int j=1;j<=binnum_gen;j++){
            Bin_size[j-1] = bins_gen[j] - bins_gen[j-1];
            Relative_Resolution[j-1]=Res_gauss[j-1]/Bin_size[j-1];
            Relative_RMS[j-1]=RMS[j-1]/Bin_size[j-1];
            Ratio_res->SetBinContent(j,Relative_Resolution[j-1]);
            Ratio_RMS->SetBinContent(j,Relative_RMS[j-1]);
    }
    Ratio_res->GetXaxis()->SetTitle("Mttbar [GeV]");
    Ratio_res->GetYaxis()->SetTitle("Ratio");
    Ratio_res->GetYaxis()->SetRangeUser(0,1.4);
    Ratio_res->SetLineColor(kBlack);
    Ratio_res->Draw();
    Ratio_RMS->SetLineColor(kRed);
    Ratio_RMS->Draw("same");

    TLegend leg3(.5, .83, .9, .9, "");
//    leg2.SetFillColor(0);
    leg3.AddEntry(Ratio_res, "Resolution (Gauss)");
    leg3.AddEntry(Ratio_RMS, "Resolution (RMS)");
    leg3.Draw("same");

    Gaus->Print("Gauss.pdf");
*/
}
