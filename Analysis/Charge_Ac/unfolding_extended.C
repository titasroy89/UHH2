...
void unfolding_extended(string pdf_name, Int_t N = 1000,Int_t bin = 2)
{
...
// Closure Test LCurve_min
      ...
      c5->Update();
      c5->Draw();
      c5->Print(Form("Closure_L_%s.pdf",pdf_name.c_str()));

      TCanvas *C3 = new TCanvas("C3","C3");
      C3->SetGridy();

      Float_t recA2[] = {-2.5,-2,-1.75,-1.5,-1.25,-1.,-.75,-.5,-.25,0.,.25,0.5,.75,1,1.25,1.5,1.75,2,2.5}; 
      Int_t rec2 = sizeof(recA2)/sizeof(Float_t) - 1; 
      Float_t recA[] = {-2.25,-1.75,-1.25,-.75,-.25,.25,.75,1.25,1.75,2.25}; 
    ...
    for(int cc=1;cc<=gen_bin;cc++){
     ...
     for (int i=0;i<gen_bin;i++){ 
     cout << bias_R[i] << "        " << bias[i] << endl;
     }
...
//     gr->GetYaxis()->SetRangeUser(-0.5,0.5);
     ...
     gr->Draw("ap");
     C4->Print(Form("Bias_L_%s.pdf",pdf_name.c_str()));
