...
#syst
'''
pu_up = unfolding_input_data.Get("ttbar_pu_up")
pu_down = unfolding_input_data.Get("ttbar_pu_down")
cferr1_up = unfolding_input_data.Get("ttbar_cferr1_up")
cferr1_down = unfolding_input_data.Get("ttbar_cferr1_down")
cferr2_up = unfolding_input_data.Get("ttbar_cferr2_up")
cferr2_down = unfolding_input_data.Get("ttbar_cferr2_down")
hf_up = unfolding_input_data.Get("ttbar_hf_up")
hf_down = unfolding_input_data.Get("ttbar_hf_down")
hfstats1_up = unfolding_input_data.Get("ttbar_hfstats1_up")
hfstats1_down = unfolding_input_data.Get("ttbar_hfstats1_down")
hfstats2_up = unfolding_input_data.Get("ttbar_hfstats2_up")
hfstats2_down =	unfolding_input_data.Get("ttbar_hfstats2_down")
jes_up = unfolding_input_data.Get("ttbar_jes_up")
jes_down = unfolding_input_data.Get("ttbar_jes_down")
lf_up = unfolding_input_data.Get("ttbar_lf_up")
lf_down = unfolding_input_data.Get("ttbar_lf_down")
lfstats1_up = unfolding_input_data.Get("ttbar_lfstats1_up")
lfstats1_down = unfolding_input_data.Get("ttbar_lfstats1_down")
lfstats2_up = unfolding_input_data.Get("ttbar_lfstats2_up")
lfstats2_down = unfolding_input_data.Get("ttbar_lfstats2_down")
MuonID_up = unfolding_input_data.Get("ttbar_MuonID_up")
MuonID_down = unfolding_input_data.Get("ttbar_MuonID_down")
Trigger_up = unfolding_input_data.Get("ttbar_Trigger_up")
Trigger_down = unfolding_input_data.Get("ttbar_Trigger_down")
'''
backgrounds = {}
...
#for name in ["pu"]:
#    systematics[name] = Systematic(name,unfolding_input_data.Get("Migration_Matrix_"+name))
...
for name, systematic in systematics.iteritems():
    unfold.AddSysError(systematic.response,systematic.name,TUnfold.kHistMapOutputVert,TUnfoldDensity.kSysErrModeMatrix)
...
'''
a=0
gg = TCanvas("gg", "gg", 700, 700)
for name, systematic2 in systematics2.iteritems():
    unfold.SetInput(systematic2.response)
    unfold.DoUnfold(tau)
    result2 = unfold.GetOutput("result2") 
    gg.cd()
    a = a+1
    result2.GetYaxis().SetRangeUser(0,12000) 
    result2.SetLineColor(a*2) 
    result2.Draw("same") 
gg.Print("gg.pdf")
'''


fout = TFile('/nfs/dust/cms/user/hugobg/UHH2/CMSSW_10_2_10/src/UHH2/Analysis/Charge_Ac/Syst_unfolded.root', 'recreate')
...
for name in ["pu","cferr1","cferr2","hf","hfstats1","hfstats2","jes","lf","lfstats1","lfstats2", "Trigger", "MuonID"]:
    ...
    gDirectory.cd('/nfs/dust/cms/user/hugobg/UHH2/CMSSW_10_2_10/src/UHH2/Analysis/Charge_Ac/Syst_unfolded.root:/')
...
'''
sys = TFile("Syst_unfolded.root")

for sy in ["pu","cferr1","cferr2","hf","hfstats1","hfstats2","jes","lf","lfstats1","lfstats2", "Trigger", "MuonID"]:
    for y in range(1,expectation.GetNbinsX()):
        mean = expectation.GetBinContent(y)
        hup = sys.Get(sy+"_up") 
        hdown = sys.Get(sy+"_down")
        up = hup.GetBinContent(y)
        down = hdown.GetBinContent(y)
        print((mean-up)/mean)
        print((mean-down)/mean)
...
totUnc.GetXaxis().SetTitle("Top quark p_{T} [GeV]")
totUnc.GetYaxis().SetTitle("Uncertainty [%]")
totUnc.GetYaxis().SetRangeUser(0.0,100.0)
