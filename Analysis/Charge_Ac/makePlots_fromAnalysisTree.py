...
print argv[1]
_fileDir ="/nfs/dust/cms/user/hugobg/Zprime_102X/analysis_output/"+argv[1]+"/muon/workdir_Zprime_Analysis_muon/"
...
histograms = {
              ...
              "pT_ttbar": ["pT_ttbar", "Events", 20, [0,1300]],
              "tlead_pT": ["tlead_pT", "Events", 20, [0,1300]]     
}
...
for sample in sample_names:
		...
		tree2_MC[sample]=_file[sample].Get("AnalysisTree")
                if(sample == "TTbar"):
			tree2_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(ttagN == 0 & btagN == 1)*weight*weight_pu*weight_btagdisc_central*weight_sfmu_Trigger*weight_sfmu_HighPtID*0.8")   
                else:
	                tree2_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(ttagN == 0 & btagN == 1)*weight*weight_pu*weight_btagdisc_central*weight_sfmu_Trigger*weight_sfmu_HighPtID")
...
tree.Draw("%s>>dat_hist(%i,%i,%f)"%(histName,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(ttagN == 0 & btagN == 1)*1")
